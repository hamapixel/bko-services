...
Le bouton **Créer les modèles mensuels manquants** prépare trois plans de 30 jours : Essentiel (demandes normales, 1 intervention en cours), Plus (normales et urgentes, 3 interventions en cours) et Pro (normales et urgentes, interventions en cours illimitées). Ils restent inactifs et à 0 FCFA tant que le superadmin n'a pas défini les prix de chacun dans **Modifier le plan**. Le serveur refuse l'activation d'un plan `mensuel-*` dont le prix est nul. Le formulaire **Créer un plan** permet aussi de créer un plan personnalisé ou de modifier prix, durée, droits, limite d'interventions simultanées, description et visibilité.

Le matching exclut un prestataire dès que ses interventions attribuées encore en cours atteignent la limite de son plan. L'acceptation revérifie cette limite sous verrou du prestataire. Sont considérées en cours les interventions acceptées, en route, sur place, en cours ou contestées. Une intervention marquée terminée par le prestataire libère sa place, même si le client n'a pas encore confirmé la fin. Aucune limite mensuelle de demandes n'est sous-entendue : il s'agit d'une capacité de travail simultané.
